(function(){const s=document.createElement("link").relList;if(s&&s.supports&&s.supports("modulepreload"))return;for(const o of document.querySelectorAll('link[rel="modulepreload"]'))n(o);new MutationObserver(o=>{for(const t of o)if(t.type==="childList")for(const i of t.addedNodes)i.tagName==="LINK"&&i.rel==="modulepreload"&&n(i)}).observe(document,{childList:!0,subtree:!0});function e(o){const t={};return o.integrity&&(t.integrity=o.integrity),o.referrerPolicy&&(t.referrerPolicy=o.referrerPolicy),o.crossOrigin==="use-credentials"?t.credentials="include":o.crossOrigin==="anonymous"?t.credentials="omit":t.credentials="same-origin",t}function n(o){if(o.ep)return;o.ep=!0;const t=e(o);fetch(o.href,t)}})();function L(a){const s=Array.from(new Set(a.map(t=>t.name?.trim()).filter(Boolean))).sort(),e=new Set;a.forEach(t=>{t.programs?.forEach(i=>{const c=i.location?.title;c&&e.add(c)})});const o=Array.from(e).sort().map(t=>`<option value="${t}">${t}</option>`).join("");return`
  ...
  `}const v="/CUOpenDayTest/cu-logo.svg";function $(a,s=null){if(s!=null){const e=a.find(t=>t.id===s);if(!e)return"";const n=e.description||"No description available.",o=(e.programs||[]).map(t=>{if(!t?.title)return"";const i=t.start_time?new Date(t.start_time).toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"}):"",c=t.end_time?new Date(t.end_time).toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"}):"",l=t.room||"-",d=t.location?.title||"-",u=t.topicImage||e.cover_image||"",p=[t.location?.accessible?'<span class="material-symbols-outlined text-blue-600">accessible</span>':"",t.location?.parking?'<span class="material-symbols-outlined text-yellow-600">local_parking</span>':"",t.location?.bike_parking?'<span class="material-symbols-outlined text-green-600">directions_bike</span>':""].join(" ");return`
  ...
  `}function _(a,s){const e=document.getElementById("topicFilter").value,n=document.getElementById("locationFilter").value,o=document.getElementById("accessibilityFilter").value,t=document.getElementById("timeFilter").value,i=document.getElementById("filterSearchInput").value.toLowerCase();return(a.topics||[]).map(l=>{const d=(l.programs||[]).filter(r=>{const f=r.location,y=f?.title===n,h=o==="wheelchair"&&f?.accessible==1||o==="parking"&&f?.parking==1||o==="bike_parking"&&f?.bike_parking==1,w=!t||new Date(r.start_time).getHours()>=parseInt(t.split(":")[0]),k=!i||r.title?.toLowerCase().includes(i)||r.description?.toLowerCase().includes(i)||r.description_short?.toLowerCase().includes(i);return(!n||y)&&(!o||h)&&(!t||w)&&(!i||k)}),u=!e||l.name===e,p=l.name?.toLowerCase().includes(s)||l.description?.toLowerCase().includes(s)||l.description_short?.toLowerCase().includes(s)||d.some(Boolean);return u&&p&&d.length>0?{...l,programs:d}:null}).filter(Boolean)}function F(a){return document.getElementById("topicFilter").value="",document.getElementById("locationFilter").value="",document.getElementById("accessibilityFilter").value="",document.getElementById("timeFilter").value="",document.getElementById("filterSearchInput").value="",a.topics.filter(s=>(s.programs||[]).length>0)}let I="",b=!1,m=null,x=[];function S(a){x=a}function C(){const a=document.getElementById("applyFilters"),s=document.getElementById("clearFilters"),e=document.getElementById("toggleView");a?.addEventListener("click",B),s?.addEventListener("click",D),e?.addEventListener("click",()=>{b=!b,m=null,g();const n=e.querySelector("span");n&&(n.textContent=b?"grid_view":"menu_book")})}function B(){const a=window.openDayData;x=_(a,I.toLowerCase()),m=null,g()}function D(){const a=window.openDayData;x=F(a),m=null,g()}function g(){const a=document.getElementById("cardsContainer"),s=m!=null?x.filter(e=>e.id===m):x;if(b&&m==null){a.className="max-w-screen-xl mx-auto px-4 sm:px-6 lg:px-8";const e=s.flatMap(n=>n.programs.map(o=>({...o,topic_id:n.id,topicName:n.name,topicDescription:n.description,topicImage:n.cover_image})));a.innerHTML=E(e),document.querySelectorAll("[data-open-id]")?.forEach(n=>{n.addEventListener("click",()=>{m=parseInt(n.getAttribute("data-open-id")),g()})})}else a.className="max-w-7xl mx-auto grid gap-6 sm:grid-cols-2 lg:grid-cols-3 px-4 sm:px-6 lg:px-8",a.innerHTML=$(s,m),document.querySelectorAll("[data-open-id]")?.forEach(e=>{e.addEventListener("click",()=>{m=parseInt(e.getAttribute("data-open-id")),g()})}),document.querySelector("[data-back]")?.addEventListener("click",()=>{m=null,g()})}function T(a,s,e){const n=document.querySelector("#app"),o=s.toLowerCase(),t=(a.topics||[]).filter(i=>{const c=i.name?.toLowerCase()||"",l=i.description?.toLowerCase()||"",d=i.description_short?.toLowerCase()||"",u=c.includes(o)||l.includes(o)||d.includes(o),p=(i.programs||[]).some(r=>{const f=r?.title?.toLowerCase()||"",y=r?.room?.toLowerCase()||"";return f.includes(o)||y.includes(o)});return u||p});e(t),n.innerHTML=`
