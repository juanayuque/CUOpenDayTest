(function(){const s=document.createElement("link").relList;if(s&&s.supports&&s.supports("modulepreload"))return;for(const a of document.querySelectorAll('link[rel="modulepreload"]'))n(a);new MutationObserver(a=>{for(const t of a)if(t.type==="childList")for(const i of t.addedNodes)i.tagName==="LINK"&&i.rel==="modulepreload"&&n(i)}).observe(document,{childList:!0,subtree:!0});function e(a){const t={};return a.integrity&&(t.integrity=a.integrity),a.referrerPolicy&&(t.referrerPolicy=a.referrerPolicy),a.crossOrigin==="use-credentials"?t.credentials="include":a.crossOrigin==="anonymous"?t.credentials="omit":t.credentials="same-origin",t}function n(a){if(a.ep)return;a.ep=!0;const t=e(a);fetch(a.href,t)}})();function L(o){const s=Array.from(new Set(o.map(t=>t.name?.trim()).filter(Boolean))).sort(),e=new Set;o.forEach(t=>{t.programs?.forEach(i=>{const c=i.location?.title;c&&e.add(c)})});const a=Array.from(e).sort().map(t=>`<option value="${t}">${t}</option>`).join("");return`
  ...
  `}const v="/CUOpenDayTest/cu-logo.svg";function $(o,s=null){if(s!=null){const e=o.find(t=>t.id===s);if(!e)return"";const n=e.description||"No description available.",a=(e.programs||[]).map(t=>{if(!t?.title)return"";const i=t.start_time?new Date(t.start_time).toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"}):"",c=t.end_time?new Date(t.end_time).toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"}):"",l=t.room||"-",d=t.location?.title||"-",u=t.topicImage||e.cover_image||"",p=[t.location?.accessible?'<span class="material-symbols-outlined text-blue-600">accessible</span>':"",t.location?.parking?'<span class="material-symbols-outlined text-yellow-600">local_parking</span>':"",t.location?.bike_parking?'<span class="material-symbols-outlined text-green-600">directions_bike</span>':""].join(" ");return`
        <article class="event-card">
          ${u?`<img src="${u}" alt="Cover image" class="event-card-image" />`:""}
          <h3 class="event-card-title">${t.title}</h3>
          <p class="event-card-meta">${i}${c?` - ${c}`:""}</p>
              ...
              <div class="flex-1 p-4 flex flex-col gap-1 sm:max-w-3xl md:max-w-4xl lg:max-w-5xl">
                <div class="flex justify-between items-start">
                  <div>
                    ...
                    <p class="text-sm text-gray-600 dark:text-gray-300">${i}</p>
  ...
  `}function _(o,s){const e=document.getElementById("topicFilter").value,n=document.getElementById("locationFilter").value,a=document.getElementById("accessibilityFilter").value,t=document.getElementById("timeFilter").value,i=document.getElementById("filterSearchInput").value.toLowerCase();return(o.topics||[]).map(l=>{const d=(l.programs||[]).filter(r=>{const f=r.location,y=f?.title===n,h=a==="wheelchair"&&f?.accessible==1||a==="parking"&&f?.parking==1||a==="bike_parking"&&f?.bike_parking==1,w=!t||new Date(r.start_time).getHours()>=parseInt(t.split(":")[0]),k=!i||r.title?.toLowerCase().includes(i)||r.description?.toLowerCase().includes(i)||r.description_short?.toLowerCase().includes(i);return(!n||y)&&(!a||h)&&(!t||w)&&(!i||k)}),u=!e||l.name===e,p=l.name?.toLowerCase().includes(s)||l.description?.toLowerCase().includes(s)||l.description_short?.toLowerCase().includes(s)||d.some(Boolean);return u&&p&&d.length>0?{...l,programs:d}:null}).filter(Boolean)}function F(o){return document.getElementById("topicFilter").value="",document.getElementById("locationFilter").value="",document.getElementById("accessibilityFilter").value="",document.getElementById("timeFilter").value="",document.getElementById("filterSearchInput").value="",o.topics.filter(s=>(s.programs||[]).length>0)}let S="",b=!1,m=null,x=[];function C(o){x=o}function I(){const o=document.getElementById("applyFilters"),s=document.getElementById("clearFilters"),e=document.getElementById("toggleView");o?.addEventListener("click",B),s?.addEventListener("click",D),e?.addEventListener("click",()=>{b=!b,m=null,g();const n=e.querySelector("span");n&&(n.textContent=b?"grid_view":"menu_book")})}function B(){const o=window.openDayData;x=_(o,S.toLowerCase()),m=null,g()}function D(){const o=window.openDayData;x=F(o),m=null,g()}function g(){const o=document.getElementById("cardsContainer"),s=m!=null?x.filter(e=>e.id===m):x;if(b&&m==null){o.className="max-w-screen-xl mx-auto px-4 sm:px-6 lg:px-8";const e=s.flatMap(n=>n.programs.map(a=>({...a,topic_id:n.id,topicName:n.name,topicDescription:n.description,topicImage:n.cover_image})));o.innerHTML=E(e),document.querySelectorAll("[data-open-id]")?.forEach(n=>{n.addEventListener("click",()=>{m=parseInt(n.getAttribute("data-open-id")),g()})})}else o.className="max-w-7xl mx-auto grid gap-6 sm:grid-cols-2 lg:grid-cols-3 px-4 sm:px-6 lg:px-8",o.innerHTML=$(s,m),document.querySelectorAll("[data-open-id]")?.forEach(e=>{e.addEventListener("click",()=>{m=parseInt(e.getAttribute("data-open-id")),g()})}),document.querySelector("[data-back]")?.addEventListener("click",()=>{m=null,g()})}function T(o,s,e){const n=document.querySelector("#app"),a=s.toLowerCase(),t=(o.topics||[]).filter(i=>{const c=i.name?.toLowerCase()||"",l=i.description?.toLowerCase()||"",d=i.description_short?.toLowerCase()||"",u=c.includes(a)||l.includes(a)||d.includes(a),p=(i.programs||[]).some(r=>{const f=r?.title?.toLowerCase()||"",y=r?.room?.toLowerCase()||"";return f.includes(a)||y.includes(a)});return u||p});e(t),n.innerHTML=`
  ...
  `,I(),g()}function A(){const o=document.createElement("header");o.className="header",o.innerHTML=`
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="flex flex-wrap justify-between items-center h-auto gap-2 py-2">
        ...
        <div class="flex items-center gap-2 min-w-0">
          <a href="https://www.cardiff.ac.uk/" target="_blank" rel="noopener noreferrer">
          <img src="${v}" alt="Cardiff University Logo" class="h-8 w-auto flex-shrink-0" />
          </a>
